...
Recently, Vaswani et al. (2017) debuted a novel, non-recurrent neural network architecture composed purely of 
self-attention called the Transformer.
The Transformer has attained state-of-the-art results in many sequence-to-sequence natural language processing tasks, 
such as machine translation.
Since the Transformer architecture lacks recurrent structure, it can be more amenable to learning long-range 
dependencies over sequences while also improving training and inference speed.

As shown below, the Transformer employs multi-headed self-attention, in which multiple attention layers run in parallel.
Intuitively, this can enable different heads to focus on different parts of the sequence.
...
Formally, multiple heads of Queries $$Q$$, Keys $$K$$ of dimension $$d_k$$, and Values $$V$$ can be packed together into separate matrices  
to allow for attention to be computed efficiently using the scaled dot-product variant, which they suggest prevents diminished gradients during training:

<figure id="attneq" class="l-body-outset">
  <div class="row">
    <div class="col one">
      <img src="{{ 'assets/img/notes/lecture-16/attneq.png' | relative_url }}" />
    </div>
   </div>
</figure>
...
<figure id="multiheadattneq" class="l-body-outset">
  <div class="row">
    <div class="col one">
      <img src="{{ 'assets/img/notes/lecture-16/multiheadattneq.png' | relative_url }}" />
    </div>
   </div>
</figure>
...
In BERT (Bidirectional Encoder Representations from Transformers), Devlin et al. (2018) used the Transformer architecture to great advantage in order to obtain *contextualized* word 
embeddings.
Despite requiring a massive amount of compute power, the authors show that the embeddings can be used out-of-the-box to 
perform at the state-of-the-art for eleven NLP tasks.

Most past work involving word embeddings (e.g. *word2vec*) learns a vector representation of a word using a simple feedforward network to predict a 
particular word's context.
Thus, even if a word has multiple meanings dependent on context, it will only receive a single embedding vector. 
On the other hand, BERT, similar to OpenAI's ELMo (Peters et al., 2018), builds a *contextual* word embedding by encoding a word  by taking into account the particular sentence context 
prior to making a prediction.
In particular, BERT uses a deep bidirectional Transformer to efficiently encode each word in a sentence into a 
contextualized representation before predicting a random masked word from the original sentence (a.k.a. the masked language model objective):
 
<figure id="BERT" class="l-body-outset">
  <div class="row">
    <div class="col one">
      <img src="{{ 'assets/img/notes/lecture-16/BERT.png' | relative_url }}" />
        <figcaption>
        Image Credit: The Illustrated Transformer by Jay Alammar.
      </figcaption>
    </div>
   </div>
</figure>

In the paper, the authors report using both Wikipedia data (2.5B words) and eBook data (800M words) for training an 
