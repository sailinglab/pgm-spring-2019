...
**Overview**: RBMs are represented by undirected bipartite graphs, with one "visible" layer a second "hidden" layer. There are no connections between nodes in the same layer (in contrast to *unrestricted* Boltzmann machines, which permit connections between hidden units).
...
- The first term is the expectation taking over the conditional of the hidden given the observed, ie. we *average over the posterior*. This is computed via sampling, which is exact (the RBM factorizes over $h$ given $v$).  In neural network literature, this is also called **the clamped**/ **wake**/ **positive phase**.
...
<figure id="DBN">
      <img width="50%" height="50%" src="{{ 'assets/img/notes/lecture-15/DBN.png' | relative_url }}" />
</figure>
