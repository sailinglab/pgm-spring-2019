...
So, $\nabla E_d [\bar{w}] = - (t_d - o_d) o_d (1-o_d) \bar{x_d}$. Thus, the incremental perception learning algorithm does the follwing (with a learning rate of $\eta$):
...
There is no direct target for these hidden units. So, we train the weights in these layers using backpropagation, or "reverse-mode differentiation". First, we can build a computaitonal graph, where $x$ is the input and $f(x)$ is the output, and nodes 2, 3, and 4 are the intermediate computations:
...
While the two paradigms appear to share the same the graphical structure on the surface, juxtaposing their properties point out subtle differences between them. 
...
RBMs are represented via undirected bipartite graph wih one layer representing the visible layer and the other representing the hidden nodes.
Learning involves computing a loss function is marginalized over all the hidden variables and taking the derivative wrt weights.
...
Of the two terms in the loss above, the first term represents the expectation taking over the conditional of the hidden given the observed, ie. we average over the posterior whereas the next term is the expectation taken over all random variables(both hidden and observed) which is equivalent to averaging over the joint. 
Both the expectations are approximated via sampling. However, while sampling from the posterior is exact, it is non trivial in the case of the joint and requires employing the iterative Gibbs sampling algorithm.
Neural network literature denotes computing the first term as the positive/ *wake* phase and the second term as the negative/ *sleep* phase.

## 2. Sigmoid Belief Networks

SBNs consist of stacks of directed linear layers combined via non linear sigmoid activation functions. The intermediate meaning of the hidden layers are typically ignored and so even though they synactically represent graphical models, operationally they begins to deviate. Operationally treating them as graphical models, leads to inefficient inference because of the implicit "explaining away" phenomenon as when we want to infer a hidden variable, it is coupled with all other hidden random variables in that layer. This stand in contrast to the RMB case because d seperation in the RBM graph meant that the hidden random variables are independent given the other layer. Also, unlike the RBM, computing the conditional does not involve a negative phase.


We now proceed to tie our discussions of inference in the SBN and the RBM. Recall that for the gradient of the sleep phase of loss function, we could sample the joint using Gibbs procedure which alternates between different subsets of varaibles. While the vanilla Gibbs sample requires sampling every single random variable one by one given all other points, a broader idea, called block Gibbs sampling which groups the variables into arbitrary blocks and samples on the block given everything else. In the case of the RBM, we consider the two layers as two blocks and hence, sampling involves alternating between the two layers. 
...
## Concluding remarks on Graphical Models v/s Deep Networks
  - Classical graphical models are concerned with the correctness of learning and inference of all variables
  - On the other hand Deep networks are more concerned about representing the observed variables using the hidden variables. Hence the focus here is somewhat different:
