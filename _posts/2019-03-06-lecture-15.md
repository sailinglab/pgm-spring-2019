...
Both the expectations are approximated via sampling. However, while sampling from the posterior is exact, it is non trivial in the case of the joint and requires employing the iterative Gibbs sampling algorithm. Consequently, this process requires extra computation and converges slowly. Neural network literature denotes computing the first term as the positive/ *wake* phase and the second term as the negative/ *sleep* phase.
...
**Combining Sequential NNs and GMMS**: 