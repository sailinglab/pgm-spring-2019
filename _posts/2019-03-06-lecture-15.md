...
where v are the visible variables and $h^1$, $h^2$ and $h^3$ are the hidden variables as shown in the figure below. Here, $P(h^2,h^3)$ is an RBM and the conditionals P(h^1|h^2)$ and $P(v|h^1)$ are represented as sigmoid activations over a linear layer. As described earlier, directly performing inference and maximizing the likelihood of the joint is problematic. Hence we resort to a greedy layer-wise pretraining strategy described below:
...
The principle of greedy layer-wise unsupervised pre-training can be described as follows:
  ...
  - **Obtain the hidden layer samples $p(h^1|h^0)$** : Use the trained RBM to get corresponding hidden samples for each data point by computing $p(h^1|h^0)$ for each point in the training set. 
...
<figure id="Pre-training" class="l-body-outset">
      <img src="{{ 'assets/img/notes/lecture-15/pretrain.png' | relative_url }}" />
</figure>
### Fine-tuning
