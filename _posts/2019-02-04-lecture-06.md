...
<figcaption>
<strong>An example Hidden Markov Model which has been unrolled to T timesteps.
</figcaption>
</figure>
...
#### EM for HMM 
For an HMM with $N$ hidden states and with $T$ observations, computing the total observation likelihood would be computationally expensive, as there are $N^T$ sequences. This can be computed efficiently using the baum welch algorithm. 
