...
$$
p(y_t^j=1|y_{t-1}^i = 1) = a_{i,j} = \mathbf{A}
$$
...
&= \texttt{log } \Pi_n (p(y_1^n) \Pi_{t=2}^T p(y_t^n | y_{t-1}^n) \Pi_{t=1}^T p(x_t^n | y_t^n)) \\
\end{aligned}
$$
...
&= \texttt{log } p(\mathbf{x},\mathbf{y};\theta) \\
&= \Sigma_n ( \langle y_1^n \rangle \texttt{log } \pi_i ) + \Sigma_n \Sigma_t \texttt{tr} ( \langle y_t^n y_{t-1}^n \rangle \texttt{log } \mathbf{A} ) + \Sigma_n \Sigma_t \langle y_t^n \rangle \texttt{log} b(x_t^n) \\
\end{aligned}
$$
...
$$
\pi^{ML} = (\Sigma_n \gamma_1^n) / N
$$

$$
A^{ML} = (\Sigma_n \Sigma_{t=2}^T \xi_t^n) / (\Sigma_n \Sigma_{t=1}^{T-1} \gamma_t^n)
$$

$$
b^{ML} = (\Sigma_n \Sigma_{t=2}^T \xi_t^n) / (\Sigma_n \Sigma_{t=1}^{T-1} \gamma_t^n)
$$
...
>>>>>>> 8c27d6b95904be2e01e3f997a31aff0513f6f882
