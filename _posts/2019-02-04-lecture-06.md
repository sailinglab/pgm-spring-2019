 ...
 **"E-step"** : We assign each data point to the closest cluster center.

$$
r_{nk} =\begin{cases}
1, & \text{if} k = \text{arg min}_j \norm{x_n - \mu_j}^2\\
0, & \text{otherwise}.
\end{cases}
$$

**"M-step"** : We set $\mu_k$ equal to the mean of all data points assigned to cluster $k$. 
...
The E-step and M-step are iterated until there is no further change in cluster assignments, or until some maximum number of iterations is exceeded. Each iteration reduces the objective funciton $J$, so the algorithm is guaranteed to converge, though not necessarily to a local optimum (recall that $J$ is non-convex).
