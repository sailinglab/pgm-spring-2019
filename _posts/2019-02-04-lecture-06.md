...
or
$$
p(y_t^j=1|y_{t-1}^i = 1) ~ \texttt{Multinomial}(a_{i,1},\ldots,a_{i,m}), \forall i \in I
$$
...
$$
p(x_t|y_t^i=1) ~ \texttt{Multinomial}(b_{i,1},\ldots,b_{i,K}), \forall i \in I 
$$
- Start or prior probabilities 
$$
p(y_1) ~ \texttt{Multinomial}(\Pi_1,\ldots,\Pi_m)
$$
