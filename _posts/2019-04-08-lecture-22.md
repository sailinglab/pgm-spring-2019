...
### From GMM to Baysian Mixture Model
We first start from the formulation of a Mixture of Gaussian (over infinite data) for the clustering problem:

\begin{align*}
P(x_1,\dots,x_N|\pi,\{\mu_k\},\{\Sigma_k\})\\
= & \prod_{n=1}^\infty \sum_{k=1}^K\pi_k\mathcal{N}\left(x_k|\mu_k,\Sigma_k\right)
\end{align*}

This above GMM is known as a parametric model. We estimate fixed number of parameters from the data. 

A baysian way of estimating the mixing weights and mixture parameters is to put a prior on them and integrate them out and then do MLE.
\begin{align*}
P(x_1,\dots,x_N)\\
= &P(x_1,\dots,x_N|\pi,\{\mu_k\},\{\Sigma_k\})\\
= &\int\int\int\prod_{n=1}^\infty \sum_{k=1}^K\pi_k\mathcal{N}\left(x_k|\mu_k,\Sigma_k\right)p(\pi)p(\mu_{1:k})p(\Sigma_{1:K}) d\pi d\mu_{1:k} d\Sigma_{1:K}
\end{align*}
