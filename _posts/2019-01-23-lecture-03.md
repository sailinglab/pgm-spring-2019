...
* (c), also called the "V-structure", is a special case. Opposite from the first two exmaples, the ball can go between $$X$$ and $$Z$$ if the node $$Y$$ is shaded, and is blocked otherwise. Therefore, the graph on the right yields $$X \perp Z \mid Y$$.
...
### Undirected Graphical Model Definition
...
Where $$Z$$ is also known as the partition function. Upper case $$C$$ is the set of all cliques. Lower case $$c$$ is a clique associated with a set of random variables $$\bold{x}$$.

Definition: an **undirected graphical model** represents a distribution $$P(X_1, ..., X_n)$$ defined by an undirected graph $$H$$, a set of positive potential functions $$\psi_C$$ and the associated cliques of $$H$$, such that
<d-math block>
\begin{aligned}
P(X_1,...,X_n)=\frac{1}{Z}\prod_{c\in C}{\psi_c(\bold{x_c})} \\
...
Even if we use fine-grained representation, the Markov network is often overparameterized. For any given distribution, there are multiple choices of parameters to describe in the model. For example, given a pair of cliques $$\{A, B\}$$ and $$\{B, C\}$$, the information about $$B$$ can be placed in either of the two cliques, resulting in ambiguities.
