...
Also, for certain conditions derived by <d-cite key="yu2013decomposing"></d-cite>, $P^\eta_{g_1+g_2}(w) = p^\eta_{g_1}(p^\eta_{g_2}(w))$, allowing us to model combinations of constraints/regularizations. 
...
Given a collection of text documents $$\mathcal{D} = \{\bm{w}_d\}_{d=1}^D$$,we denote $$\bm{w}_d$$ as the $$d$$th document in the corpus, and $$\bm{w}_d = \{w_{di}\}_{i=1}^{N_d}$$ with $$w_{di}$$ being the $$i$$th word in $$\bm{w}_d$$, and $$N_d$$ as the number of words in $$\bm{w}_d$$. LDA assumes each document is generated from a mixture of $$K$$ topics, with the mixture proportion $$\bm{\theta}_d$$, where each topic is a multinomial distribution parametrized by $$\bm{\beta}_{k}$$ over all $$V$$ words existed in the vocabulary.

The log-likelihood of an observation is then:
...
### Problems with Classic Parallelization

1. We can take multiple MCMC chains and run them in parallel.\
   **Disadvantage**: convergence rate stays the same, we only gain in the number of obtained samples after convergence

2. We can use Sequential Importance Sampling (SIS) to break down the distribution into $$n$$ smaller telescoping distributions that can be sampled in parallel and then later combined.\
   **Disadvantage**: variance increases exponentialy with $$n$$, therefore reducing stability.

To address these disadvantages, we will look at five more modern solutions to parallelization for MCMC algorithms. Based on the categorization we mentioned earlier, we have three *data parallel* methods, and two *model parallel* methods.

### Data-parallel 1: Introduce Auxiliary Variables <d-cite key="Williamson2013"</d-cite><d-cite key="Dubey2014"></d-cite>
...
### Data-parallel 2: Run MCMC on Subsets of Data <d-cite key="Neiswanger2013"></d-cite>
...
### Data-parallel 3: Use Collapsed Gibbs Sampler <d-cite key="Griffiths2004"></d-cite><d-cite key="Newman2009></d-cite>
...
### Model-parallel 1: GraphLab <d-cite key="Low2010"></d-cite><d-cite key="Gonzalez2012"></d-cite>
...
### Model-parallel 2: LightLDA <d-cite key="Yuan2015"></d-cite>
