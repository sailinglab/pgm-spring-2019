...
In this data-parallel method, we simply split the observations into multiple partitions, have each processors process each own partition, and then aggregate the update later. This process is illustrated below:
...
2. Copy the topic description $$B$$ to each processor
3. Each processor runs inference locally and update their parameters
4. All processors send back the update to the master server, which aggregates the update
5. Master server sends back the updated parameters to all processes, repeating the process.

The problem with this approach is two-fold:

1. The overall model is no longer guaranteed to converge, since ergodicity (fully-connectedness) is broken
2. **Bigger problem**: communication time to send parameter updates between master server and the processors could be longer than the computation time.
