...
And the algorithm is summarized below:
...
Here we discuss some considerations when designing real-world distributed systems.

**Two distributed challenges:**
1. Networks are slow
2. Identical machines rarely perform equally: Hotter machines, vibrating machines, network used by others, other users using computations, etc.

Pay attention to system aspects: bandwidth, latenty, partitioning and scheduling
(not constants but probabilities, thus non-ideal systems behavior)

<img src="{{ '/assets/img/notes/lecture-27/dichotomy.png' | relative_url }}" height="300px" />
...
1. Partial synchronicity
2. Straggler tolerance
Existing ways are either safe/slow (BSP) or fast/risky (Async)
Stale sync: the middle-ground between bulk and async

**Theory of Real Distributed ML Systems**

There are three types of convergence guarantees:
- Regret/Expectation bounds on parameters --> convergence progress per iteration
- Probabilistic bounds on parameters --> similar to the regret bound but usually stronger
- Variance bounds on parameters --> stability near optimum or convergence

E(SSP) Regret Bound:
$$
R(x)=(\frac{1}{T}f_t(\tilde{x_t}))-f(X)\leq 4FL\squrt{\frac{2(s+1)P}{T}}
$$
Note the interrelation between $$(L,F)$$ and $$(s,P)$$

Why does E(SSP) converge? The intuition is that E(SSP) approximates the sequiential ordering of the work. Despite the re-ordering, over the long run E(SSP) is equivalent to sequential execution.

Influence of each factor would differ in differnt systems, which cope with delays in differnt ways.
