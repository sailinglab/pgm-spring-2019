...
# Readings
## Koller and Friedman Ch 3 <d-cite key="Koller:2009:PGM:1795555"></d-cite>: The Bayesian network representation
...
In words, this states that _features are conditionally independent, given the class of the instance of this model_.  Thus the joint distribution of the Naive Bayes model factorizes as follows:

$$ P(C, X_1, ... X_n) = P(C) \prod_i P(X_i |C)$$

### 3.2.1 Bayesian networks
Unlike in the Naive Bayes model, Bayesian networks can represent distributions that do not satisfy the naive conditional independence assumption. 
...
* _picking variables precisely_: we need some variables that can be observed, and their domain should be specified. Sometimes introducing hidden variables will help to render them conditionally independent.
