...
A GM $M$ describes a unique probability distribution $P_{M}$. 
...
Deep Belief Network (DBN) \[hinton 2006\] is an example. 
...
With this concept in hand we can outline our new algorithm.

1. Given a query of the form $P(X_1 | e)$, we first focus on the joint probability $P(X_1, e)$.
$$P(X_1, e) = \sum_{x_n}\dots\sum_{x_2}\prod_{i} P(x_i, pa_i)$$
This suggests an implicit "elimination order" over the variables.
2. Following the order prescribed above:
  1 Move all the relevant terms to the innermost sum and all irrelevant terms out of it.
  2 Perform the Sum-Product operation on the innermost sum, producing a new factor $\phi$.
  3 Repeat until the entire joint is calculated.
3. So calculate the desired query, simply divide the joint by the marginal probability of the evidence.
$$
...
Given a Bayesian Network factorizing as the graph shown in the right, we are going to do VE to inference $P(A| h)$. The initial factors are:
$$ P(a)P(b)P(c|b)P(d|a)P(e|c,d)P(f|a)P(g|e)P(h|e,f)$$
Before doing VE we choose an elimination order as $H,G,F,E,D,C,B$. 
...
**Graph transformation:** After conditioning on $h$, we eliminate $h$ in old factors and generate a new factor $m_h(e, f)$. In the perspective of graph evolving, as shown in the right, this corresponding to delete the node $h$ as we eliminate $h$, and also add an edge between node $E$ and $F$ since the generated new factor depending on $E$ and $F$. The reduced factorization is a Gibbs distribution factorizing over the new graph. 
<figure class="l-gutter">
  <div class="row">
...
**Graph transformation:** Just remove node $G$ as the right. 
<figure class="l-gutter">
  <div class="row">
...
**Graph transformation:** Generated term <=> fully connected subgraph, according to the Gibbs distribution property. As shown in the right, node $E$ is removed and $E$’s neighbors are moralized.
<figure class="l-gutter">
  <div class="row">
...
**Graph transformation:** As in the right. 
<figure class="l-gutter">
  <div class="row">
...
The new product of factors:
$$ P(a)P(b)m_c(a, b)
** Graph transformation:** As in the right, moralize $A$ and $B$. 
<figure class="l-gutter">
  <div class="row">
...
We can define the **width** of a reconstituted graph as the size of largest clique minus 1. Let $w_{G,I}$ be the width of $G’_{I}(V, E’)$. For different ordering $I$ $w_{G,I}$ will be different. Now we define **tree-width** of $G$ as:
$$ w^*_{G} = \min_{I} w_{G,I} $$
...
It’s extremely hard to find a optimal elimination ordering. And actually the tree width of ising model is large than the $\sqrt{n}$ so even finding a optimal elimination ordering, the VE algorithm is still exponential in $n$. [Need to check video]
