...
Imagine we want the probability of $E=e$ regardless of the values of $A, B, C, D$. Na\"ively, we could sum over the joint probability:
...
Na\"ively we could find the conditional probability of $y_i$ given the observed sequence, but using our elimination trick, we can get similar computational advantages as seen in the chain example.
