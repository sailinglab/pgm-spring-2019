...
+ **Inference:** answering queries about the probability distribution $P_M$ defined by $M$, for examples, $P_M(X|Y)$ where $X$ and $Y$ are subsets of variables in GM $M$.
+ **Learning:** estimating a plausible model $M$ from data $D$.  We call the process of obtaining a point estimate of $M$ as *learning*, but for Bayesian, they seek the posterior distribution of $p(M|D)$, which is actually an *inference* problem. The learning is task is highly related to the inference task. When we want to compute a point estimate of $M$, we need to do inference to impute the missing data if not all the variables are observable. So the learning algorithm usually uses inference as a subroutine. 

### Inference Problems
Here we will study different kind queries associated with the probability distribution $P_M$ defined by GM $M$.

#### Likelihood
Most queries one may ask involve *evidence*, so we first introduce the definition of evidence. 
...
$$ P(Y | \mathbf{e}) = \sum_\mathbf{z} P(Y, Z = \mathbf{z} | \mathbf{e}). $$
The process of summing out the "don't care" variables $Z$ is called *marginalization*, 
and the resulting $P(Y | \mathbf{e})$ is called a *marginal* prob.
...
**TO-DO Figure**
In this type of queries, the query node is the descendent of the evidence. If we know the value of variable $A$ and $B$, the probability of the outcome is a posteriori belief $P(C|A,B)$. Using the conditional independence $C \perp A | B$ encoded in the graph, we can simplify it to $P(C|A,B) = P(C|B)$.
...
In this type of queries, the query node is the ancestor of the evidence.  In the GM $M$, if we know the value of variable $B$ and $C$, the probability of the cause is a posteriori belief $P(A|B,C)$. Again using the conditional independence, we can simplify it to $P(A|B,C) = P(A|B)$.
...
$$ MPA(Y | e) = arg\max_{\mathbf{y} \in \mathcal{Y} } P(\mathbf{y} | \mathbf{e}) = arg\max_{\mathbf{y} \in \mathcal{Y} } P(\mathbf{y}, \mathbf{z} | \mathbf{e}) $$.
