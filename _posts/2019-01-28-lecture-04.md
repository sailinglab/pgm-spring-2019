...
H variable node is observed node, we can add additional evidence indicator factor to make conditioning on observed evidence as isomorphic as a marginalization step: $$ m_h(e,f) = p(h=\bar{h} |e, f) = \sum_{h} p(h|e,f) \delta(h=\bar{h}).$$ The new product of factors becomes:
...
Step 3: eliminate F
Compute:
$$ m_f(a, e) = \sum_f p(f|a)m_h(e, f).$$ The new product of factors: $$ P(a)P(b)P(c|b)P(d|a)P(e|c, d)m_g(e)m_f(a, e)$$
