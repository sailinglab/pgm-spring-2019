...
We have an undirected graphical model and we know from using the Hammersley–Clifford theorem, that it can be represented by a Gibbs distribution.
Now the question arises if we can follow the same procedure as for directed graphical Models to find the MLE of a UGM?  The answer is no - we cannot.
...
However the same is not applicable to undirected graphical models, as there is a normalization constant $Z$ that  is a function of all the parameters and thus the probability distribution does not split into a sum of terms.
<d-math block>
...
_**Log Likelihood for UGMs**_:

We introduce two new quantities *Total Count* and *Clique Count*.
...
Taking the derivative we get the following terms,
...
Notice that the $\psi_{ c } \left( \mathbf { x }_{ c } \right)$ term gets cancelled and we are  left with a condition.
...
IPF, is also a co-ordinate ascent algorithm, where the co-ordinates are the parameters of the clique potentials. During each step of the algorithm the log likelihood increases and thus it converges to a global maximum.
...
Having motivated why such models might be useful, let us see how one can learn and do inference over these models.
