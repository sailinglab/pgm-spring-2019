...
Another task is text content manipulation task, the goal of the task is basically to generate a sentence to describe the content in  a given data record. Generally there are many ways to experss the content, 
however we want to generate a sentence in a specific style as like a given reference
sentence. Here is a example,we want to rephrase the statistics of LeBron James and Irvine 
into a sentence in the style of the reference sentence "player 1 lead the way with x statitics while
 player 2 ". 
 
To achieve this conThe basic idea is to jointly optimzie two objectives: 1)experss the original
