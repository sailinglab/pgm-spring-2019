...
### Case Study 1: Text attribute transfer 
...
Use encoder-decoder architecture to model $p_{\theta} (\mathbf{y} | \mathbf{x}, \mathbf{a}_y)$
...
### Sub-objectives
* Auto-encoding loss: $(\mathbf{x}, \mathbf{a}_x) \rightarrow \mathbf{x}$
* Classification loss: $\hat{\mathbf{y}} \sim p_{\theta} (\mathbf{y} | \mathbf{x}, \mathbf{a}_y)$, $f(\hat{\mathbf{y}}) \rightarrow \mathbf{a}_y)$
...
Solution: add language model (LM) as a discriminator with the objective $\max_{\theta} \text{LM}(\hat{\mathbf{y}})

###Results
* Sentiment classification accuracy: 91%
* BLEU metric (measured against input sentence): 57
* LM perplexity: 60.9


The generated sentences have similiar accuracy and BLEU scores, but a much better perplexity. See example below.

__Original:__  "uncle george is very friendly to each guest"
