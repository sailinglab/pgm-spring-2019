...
* E-step: $$q(y|x)=\delta(y=y^*)$$
* M-step: 
\begin{align*}
\theta &=\arg \max_\theta \mathbb{E}_q[ \log p_\theta(y|x)] 
= \arg \max_\theta \int_{y\in Y} q(y|x) \log p_\theta(y|x) dy
\\& =\arg \max_\theta \int_{y\in Y} \delta(y=y^*) \log p_\theta(y|x) dy 
 =\arg \max_\theta  \log p_\theta(y^*|x).
\end{align*}
...
For that assume that $Y$ is equipped with some metric $w$. Then we can retrieve the Data Noising approach from $ERPO$ by setting  $\alpha \to 0$, $\beta=1$ and 
\begin{equation*}
R(y|y^*) =
 \begin{cases}
1 &\text{if } ||y-y^*||_w \leq \epsilon,\\
-\infty &\text{otherwise}.
\end{cases}
\end{equation*}
In the case that $Y$ is discrete we may take $\epsilon$ to be a small natural number.
...
* E-step: $$q(y|x) \propto  e^{R_c(y,y^*)},$$
* M-step: 
\begin{align*}
\theta &=\arg \max_\theta \mathbb{E}_q[ \log p_\theta(y|x)] 
= \arg \max_\theta \mathbb{E}_{y \sim e^{R_c(y,y^*)}}[ \log p_\theta(y|x)].
\end{align*}
$\mathbb{E}_{y \sim e^{R_c(y,y^*)}}[ \log p_\theta(y|x)]$ is exactly the objective that RAML tries to maximize.
...
* E-step: $$q_{n+1}(y|x) \propto p_{\theta^n}(y|x) e^{R_c(y,y^*)},$$
* M-step: $$\theta^{n+1} =\arg \max_\theta \mathbb{E}_{q_{n+1}(y|x)}[ \log p_\theta(y|x)],$$
...
$$q_{n+1}(y|x) \propto \exp \bigg\{c\big(\lambda_{1,n+1} \log  p_{\theta^n}(y|x) + \lambda_{2,n+1} R_\delta(y,y^*) +\lambda_{3,n+1} R_c(y,y^*) \big)\bigg\},$$
