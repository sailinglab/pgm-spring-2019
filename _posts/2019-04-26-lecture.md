...
<d-math block>
\begin{aligned}
k_{\text{RBF}(x, x')} &= \text{cov} (f(x), f(x')) \\
&= a^2 \exp(-\frac{\Vert x-x'\Vert^2}{2\ell^2})
\end{aligned}
</d-math>
 ...
 * Learning requires a $\log | k_\theta + \sigma^2 \mathbf{I}|$ step,
...
In standard learning, given a distribution over examples (single task), we learn a function that minimizes the loss $\hat \phi = argmin_{\phi} E_{z \sim D}[l(f_{\phi}(z))]$

In learning-to-learn, given a distribution over tasks, output an adaptation rule that can be used at test time to generalize from a task description: $\hat \theta = argmin_{\theta} E_{T \sim P}[L_T(g_{\theta}(T))]$ where $L_T(g_{\theta}(T)) \coloneqq E_{z \sim D}[l(f_{\phi}(z))]$, $\phi \coloneqq g_{\theta}(T)$
