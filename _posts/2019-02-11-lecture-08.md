...
<figure>
  <div class="row">
    <div class="col two">
      <img src="{{ '/assets/img/notes/lecture-08/scribe1.jpg' | relative_url }}"  width="100" height="80" />
    </div>
  </div>
  <figcaption>
...
X and Y are dependent if ad only if $\exists x_1 \neq x_2$ such that:
...
This answers the question of $P(y_x | x', y')$, which is the probability that event Y = y would be observed had X been x, given that we actually observed X to be $x'$ and and Y to be $y'$. For this, again we require causal relations via the structural equation model.
