...
A sufficient condition under which the causal effect $P(y | do(x))$ is identifiable is that there exist no bi-directed path between X and any of its children. For more information, this is presented in the paper "A General Identification Condition for Causal Effect".
...
1. For each pair (X, Y), test $X \perp Y | \varnothing$, if the independence holds, remove the edge.
2. For each pair (X,Y) adjacent, with $\text{max} \{ |Adj(X)|, |Adj(Y)|\} \geq 2$, test $X \perp Y |S $ where $|S| = 1$ and $S \subseteq Adj(X)$ or $S \subseteq Adj(Y)$.
    \item ...
    \item For each pair (X,Y) adjacent, with $\text{max}\{|Adj(X)|, |Adj(Y)|\} \geq k+1$, test $X \perp Y |S $ where $|S| = k$ and $S \subseteq Adj(X)$ or $S \subseteq Adj(Y)$.
\end{itemize}

We stop when we reach k s.t for all (X,Y), $\text{max}\{| Adj(X)|, |Adj(Y)|\} < k+1$ (since it’s enough to find one set of nodes that make X and Y independent to remove their edge).
