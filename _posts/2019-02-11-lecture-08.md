...
* For each pair (X,Y) adjacent, with $$\text{max}  |Adj(X)|, |Adj(Y)|  \geq 2$$, test $$ X \perp Y \mid S $$ where $$ \mid S\mid = 1$$ and $$ S \subseteq Adj(X) $$ or $$ S \subseteq Adj(Y) $$.
* ...
* For each pair (X,Y) adjacent, with $$\text{max} |Adj(X)|, |Adj(Y)|  \geq k+1$$, test $$ X \perp Y \mid S $$ where $$ \mid S \mid = k $$ and $$ S \subseteq Adj(X) $$ or $$ S \subseteq Adj(Y) $$.
