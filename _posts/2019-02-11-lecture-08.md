...
The definition requires just a pair of distinct $$\inline x_1, x_2$$ to have different conditional distribution of Y. Y might have same conditional distribution over a range of x values. But if we find even a single pair for which the conditional distribution is different, X and Y are dependent. Same goes for causation 
