...
The definition requires just a pair of distinct $$ x_1, x_2 $$ to have different conditional distributions of $$Y$$. $$Y$$ might have the same conditional distribution over a range of $$X$$ values, but if we find even a single pair for which the conditional distribution is different, $$X$$ and $$Y$$ are dependent. The same goes for causation.
...
* For each pair (X,Y) adjacent, with $$\text{max}  |Adj(X)|, |Adj(Y)|  \geq 2$$, test $$X \perp Y |S $$ where $$|S| = 1$$ and $$S \subseteq Adj(X)$$ or $$S \subseteq Adj(Y)$$.
* ...
* For each pair (X,Y) adjacent, with $$\text{max} |Adj(X)|, |Adj(Y)|  \geq k+1$$, test $$X \perp Y \mid S $$ where $$|S| = k$$ and $$S \subseteq Adj(X)$$ or $$S \subseteq Adj(Y)$$.
