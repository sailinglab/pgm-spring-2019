...
* Generative model $\bm{x} = G_\theta(\bm{z}), \bm{z} \sim p(\bm{z})$
...
* Problem with VANILLA GAN
    * If data and model's manifold are different, we can have negligible intersection in practice
    * For example, for some $x$ such that $P_g(x)=0$ but $P_d(x) > 0$, KL divergence is infinite
    * The loss function and gradients may not be continous and well behaved
* Motivation and Contribution of WGAN <d-cite key="wgan"></d-cite>
    * Propose Wasserstein Distance that is well defined
...
* Learning Objective 
    * $\|D\|_{L} \leq K$ K-Lipschitz continuous
...
* WGAN training
    * Optimal discriminator and critic when learning to differentiate two Gaussians. As we can see, the traditional GAN
discriminator saturates and results in vanishing gradients. WGAN critic provides very clean gradients on all parts of the space
...
### Progressive GAN
* Problem with VANILLA GAN
    * Although GAN can produce sharp images, in most of the cases those images are in small resolutions with limited variation
    * Training continues to be unstable 
* Motivation and Contribution of Progressive GAN <d-cite key="pgan"></d-cite>
    * Train on low resolution images first, progressive increase the resolution by adding layers
    * First detect large structure of image, and then shift attention to finer details
...
### BigGAN
* Motivation of BigGAN
    * Generate high fidelity, diverse images with models directly learned from data
    * GANs benefit dramatically from scaling
    * 2x–4x more parameters, 8x larger batch size
    * Use strong discriminator first, and then relaxing it towards the end of training
