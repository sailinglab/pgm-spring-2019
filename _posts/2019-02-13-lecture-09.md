...
### Gaussian Graphical Model
  ...
  where $\Sigma^{(n)}$ encode dependencies among variables. As we mentioned before, the inverted matrix of covariance matrix is called precision matrix which encodes non-zero edges in Gaussian Graphical Model
...
<figure>
  ...
  - Problem
    - You want the precision matrix to be invertible, but non full rank does not mean MN does not exist
    - The computational complexity of inverting a matrix is $O(N^3)$ where N is the number of node in a graph
...
<figure>
  ...
  And theoretically, there have been proof that graphical lasso can recover the true structure of the graph. <d-cite key="Meinshausen"></d-cite>
 proved that if:
...
$$\hat{\theta_i^t} = argmin _{\theta_i^t} l_w(\theta_i^t) + \lambda_1||\theta_i^t||_1 \forall t $$
