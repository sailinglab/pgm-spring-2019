...
<d-math block>
\begin{aligned}
\psi_c(c_1,c_2,c_3) = \exp{\theta_{ing}f_{ing}} \times \exp{\theta_{?ed}f_{?ed}} \times ... \\
  exp{\sum^K_{k=1}\theta_k f_k(c_1,c_2,c_3)}
\end{aligned}
</d-math>

There's still a potential over $26^3$ possible settings, but only uses $K$ parameters if there are $K$ features, and by having one indicator function per combination of $x_c$, we recover the standard tabular potential. Note that these features can be handcrafted. We also append weights $\theta$ in the function because we do not have contextual knowledge of how important each feature is. 
