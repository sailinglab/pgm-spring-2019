...
We can incoporate such domain knowledge to impose structure on the RVs $$ X_{1},...,X_{8} $$.
...
<ol>
  <li>Save cost in representing the joint distribution
By modeling the dependencies among the RVs with a graph and conditionals, the number of parameters needed to decribe the joint distribution is much fewer than when using a full joint distribution table.</li>
...
$P(X_{3}|X_{1})\rightarrow 2 \text{  parameters}$ 

$P(X_{4}|X_{2})\rightarrow 2 \text{  parameters}$

$P(X_{5}|X_{2})\rightarrow 2 \text{  parameters}$ 

$P(X_{6}|X_{3}, X_{4})\rightarrow 4 \text{  parameters}$

$P(X_{7}|X_{6})\rightarrow 2 \text{  parameters}$  

$P(X_{8}|X_{5}, X_{6})\rightarrow 4 \text{  parameters}$
...
By factoring the joint distribution into modular terms, each term becomes self-contained and we can estimate each term with only the relevant datapoints (e.g. to estimate $$ P(X_{8}|X_{5}, X_{6}) $$ we only need data for $$ X_{8}, X_{5} and X_{6} $$)
...
<dl>
    ...
    <dd>- one lab can study the subtree formed by $$ X_{1}, X_{3}, X_{6}, X_{7}, X_{8} $$ and another lab can study $$ X_{2}, X_{4}, X_{5} $$, then fuse their estimations together by multiplying the terms by their dependencies.</dd>
  ...
  <li>Graphical models provide a generic method of representing knowledge and making inferences
...
Therefore, a graphical model along with the Bayes Theorem provide a universal way of representing knowledge and computation.  </li>
</ol>
