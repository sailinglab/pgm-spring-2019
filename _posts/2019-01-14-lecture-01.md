...
<ol>
  <li>Save cost in representing the joint distribution
    ...
    <dl>Assume all RVs are binary.
      <dd>$P(X_{1})\rightarrow 1 \text{  parameter}$</dd>  
      <dd>$P(X_{2})\rightarrow 1 \text{  parameter}$</dd> 
      <dd>$P(X_{3}|X_{1})\rightarrow 2 \text{  parameters}$</dd> 
      <dd>$P(X_{4}|X_{2})\rightarrow 2 \text{  parameters}$</dd>
      <dd>$P(X_{5}|X_{2})\rightarrow 2 \text{  parameters}$</dd>
      <dd>$P(X_{6}|X_{3}, X_{4})\rightarrow 4 \text{  parameters}$</dd>
      <dd>$P(X_{7}|X_{6})\rightarrow 2 \text{  parameters}$</dd>  
      <dd>$P(X_{8}|X_{5}, X_{6})\rightarrow 4 \text{  parameters}$</dd>
      <dd>Total of 18 parameters</dd>
    </dl>
...
By factoring the joint distribution into modular terms, each term becomes self-contained and we can estimate each term with only the relevant datapoints (e.g. to estimate $P(X_{8}|X_{5}, X_{6})$ we only need data for $X_{8}, X_{5}, X_{6}$)
Therefore, the problem of joint distribution estimcation can be modularized into smaller pieces and integrated later by multiplication.
  ...
  <li>Graphical models provide a generic method of representing knowledge and making inferences
    ...
    <dl>
      <dd>$p(h|d)=\frac{p(d|h)p(h)}{p(d)}$</dd>
      <dd>$p(h|d)=\text{  posterior}$</dd>
      <dd>$p(d|h)=\text{  likelihood}$</dd>
      <dd>$p(h)=\text{  prior}$</dd>
      <dd>$p(d)=\text{  marginal probability of the data}=\int_{H}p(d|h)p(h)dh$</dd>
    </dl>
