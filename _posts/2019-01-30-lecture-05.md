...
In general, each training case $\mathbf{x}_n = (x_{n,1}, ..., x_{n, M})$ is a vector of $M$ values, one per node. Depending on the model, elements in $\mathbf{x}_n$ could be all known (no missing values, no hidden variables) if the model is completely observable, or partially known ($\exists i, s.t. x_{n,i}$ is not observed) if the model is partially observable.

In this class, we mainly consider learning parameters for a **BN** with given structure and is **completely observable**.

## Exponential Family Distribution

There are various density estimation tasks that can be viewed as single-node GMs (see supplementary section for examples), and they are in fact instances of 

**Exponential Family Distrubution**. Exponential Family Distrubutions are building blocks for general GM, and have nice properties allowing us to easily do MLE and Bayesian estimation. It's 
...
$$p(x|\mu,\Sigma) = \frac{1}{(2\pi)^{k/2}|\Sigma|^{1/2}}exp\{-\frac{1}{2}(x-\mu)^T \Sigma^{-1}(x-\mu)\}$$
...
$$p(x|\pi) = \pi_1^{x_1}\pi_2^{x_2}...\pi_K^{x_K}=exp\{\sum_k x_k\ln\pi_k\}$$
...
For iid data, we have the log-likelihood to be 
