...
Let $V_t(s_t) = \log \beta_t(s_t), Q_t(s_t, a_t) = \log \beta_t(s_t, a_t), V(s_t) = \log \int exp(Q(s_t, a_t) + \log p(a_t | s_t)) da_t$. Denote $\tau = (s_1,a_1, …, s_T,a_T)$ as the full trajectory. Denote $p(\tau) = p(\tau | \mathcal{O}_{1:T})​$. Running inference in this GM allows us to compute:

$$p(\tau | \mathcal{O}_{1:T}) \propto p(s_t) \displaystyle \prod_{t=2}^T p(s_{t+1} | s_t, a_t) \times exp(\displaystyle \sum_{t=1}^T r(s_t, a_t)) \\ p(a_t | s_t, \mathcal{O}_{1:T}) \propto exp(Q_t(s_t, a_t) - V_t(s_t))​$$
...
The approach still involves explicitly optimization of $\pi_\theta$. We can rewrite the iteration as:
...
If the state space is high-dimensional, it is not feasible to represent $Q$ as a parametric function. In this case, we adopt fitted Q-iteration:
