...
layout: distill
title: Lecture 21:
description: Sequential decision making (part 2): The algorithms
date: 2019-04-03
...
$J(\theta) = -D_{KL}(\hat{p}(\tau) || p(\tau)) = \sum_{t=1}^T \mathbb{E}_{(s_t, a_t) \sim \hat{p}(s_t, a_t)}[r(s_t, a_t)] + \mathbb{E}_{s_t \sim \hat{p}(s_t)}[\mathcal{H}(\pi(a_t|s_t))]$
