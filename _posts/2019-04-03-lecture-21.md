...
<d-math block>
\begin{aligned} 
& V_\pi(s) := \mathbb{E}_\pi \displaystyle \sum_{k=0}^T \big[\gamma^k r_{t+k+1} \mid s_t = s \big]\\ 
& Q_\pi (s, a) := \mathbb{E}_\pi \displaystyle \sum_{k=0}^T \big[\gamma^k r_{t+k+1} \mid s_t = s, a_t = a \big]\\ 
& \pi_*(a \mid s) := \delta(a = \underset{a}{\mathrm{argmax}} Q_*(s,a)) 
\end{aligned}
</d-math>
...
$$-D_{KL}(\hat{p}(\tau) || p(\tau)) = \sum_{t=1}^T \mathbb{E}_{(s_t, a_t) \sim \hat p(s_t, a_t)}[r(s_t, a_t)] + \mathbb{E}_{s_t \sim \hat{p}(s_t)}[\mathcal{H}(\pi(a_t \mid s_t))]​$$
